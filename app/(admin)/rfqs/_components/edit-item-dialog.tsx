...
import { updateRFQItemSchema } from "@/lib/validations/rfq";
...
interface EditItemDialogProps {
  ...
  rfqId: string;
  item: CreateRFQResponseItem;
  onSuccess?: (updatedItem: CreateRFQResponseItem) => void;
}
...
export function EditItemDialog({
  ...
  const handleSubmit = async () => {
    ...
    const result = updateRFQItemSchema.safeParse(payload);
    if (!result.success) {
    ...
    try {
      const res = await rfqsApi.updateItem(rfqId, item.id, result.data);
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>
            Update the material, quantity, budget, or description for this item.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <Textarea
              ...
              placeholder="e.g., Updated specs: must be 80gsm white"
              rows={3}
            />
          </div>
        </div>
